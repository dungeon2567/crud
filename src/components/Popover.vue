...
.popper.tooltip > div > .popover > .popover-arrow > svg > path.popover-arrow-border {
	fill: #10161a;
	fill-opacity: 0.1;
}
...
.popper.tooltip > div > .popover > .popover-arrow > svg > path.popover-arrow-fill{
	fill: #394b59;
}
...
.fade-scale-enter-active {
	transition: opacity 0.25s cubic-bezier(0.4, 1, 0.75, 0.9), transform 0.25s cubic-bezier(0.4, 1, 0.75, 0.9);
}
...
<template>
	<div ref="popper" class="popper" v-resize="onResize">
		<transition name="fade-scale" @before-enter="doCreate" @after-leave="doDestroy">
			<div v-if="showPopper">
				<div class="popover">
					<div ref="arrow" class="popover-arrow">
...
export default {
	...
	mounted() {
		...
		switch (this.trigger) {
			case 'click':
				...
				break;
			case 'hover':
				this.referenceElm.addEventListener('mouseover', this.onMouseOver, false);
	...
	methods: {
		...
		destroyPopper() {
			this.referenceElm.removeEventListener('mouseover', this.onMouseOver, false);
